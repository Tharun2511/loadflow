...
import { AlertTriangle, CheckCircle2, Clock, Plus, ShieldAlert, Truck, Search, Filter } from "lucide-react"
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        {hasPermission('load.create') && (
          <form action={async (formData) => {
            ...
            <input type="text" name="origin" placeholder="Origin" required className="px-3 py-2 rounded-lg bg-slate-800 border border-white/10 text-sm text-white w-24 sm:w-auto" />
            <input type="text" name="destination" placeholder="Destination" required className="px-3 py-2 rounded-lg bg-slate-800 border border-white/10 text-sm text-white w-24 sm:w-auto" />
            <select name="shipperId" required className="px-3 py-2 rounded-lg bg-slate-800 border border-white/10 text-sm text-white w-24 sm:w-auto">
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between bg-slate-800/50 rounded-lg p-3 border border-white/5 gap-2">
                      ...
                      {latestRate.status === 'PENDING' && hasPermission('rate.confirm') && !load.complianceFlag && (
                        <form action={async () => {
                          ...
                          <button type="submit" className="w-full sm:w-auto px-3 py-1.5 rounded bg-emerald-500/20 hover:bg-emerald-500/30 text-emerald-400 font-medium text-xs transition-colors">
                            Confirm Rate
                          </button>
                        </form>
                      )}
